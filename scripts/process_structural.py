...
# ---------- Prepare borehole data ----------------
    ...
    if Fault:
        ...
        for point in spatial.fault_nodes:
            x,y = point[0], point[1]
            print(x,y)
            fx.append((x))
            fy.append((y))
        fz = [-100, -200, -300, -400, -500] # making a plane
...
def create_structuralmodel(structuralmodel, Fault = True):
    ...
    Quaternary         = model.create_and_add_foliation("Quaternary", nelements=1e4)
    Quaternary_UC      = model.add_unconformity(Quaternary, structuralmodel.strat[structuralmodel.strat.unit == 'QTs'].val.iloc[0]) 
    
    if Fault:
        print('Fault included!')
        Fault = model.create_and_add_fault('Fault', displacement = 400,)
        
    model.create_and_add_foliation("Lancelin", nelements=1e4, buffer=0.1)
    model.create_and_add_foliation("Warnbro", nelements=1e4, buffer=0.1)
    model.create_and_add_foliation("Yarragadee", nelements=1e4, buffer=0.1)
